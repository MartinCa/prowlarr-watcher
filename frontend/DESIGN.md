...
- **What this app is:** Prowlarr Watcher — a small web app that periodically searches
  ...
  queries and sends notifications via Apprise when genuinely new results appear.
- **Who uses it:** A single trusted operator, self-hosted on a private network or
  behind a reverse proxy. There is no authentication by design (see repo README) —
  the app is not exposed to the internet.
- **Router / framework choice and why:** Vite + TanStack Router (SPA). No SSR/SEO
  need — this is a single-operator internal tool.
- **Backend and where its OpenAPI spec lives:** Flask (Python), served from the same
  origin at `/api/*`. The spec is hand-authored at `frontend/openapi.json` (Flask has
  no framework-native OpenAPI generation like FastAPI/ASP.NET Core) and kept in sync
  with `routes.py` by hand; `src/lib/api-types.ts` is generated from it via
  `openapi-typescript` and is vendored like shadcn components — never hand-edited.
- **Pagination convention:** None. Result sets are small (one operator's queries and
  their search results) — list endpoints return the full collection.
- **Deviations from the shared conventions (with reasons):**
  - CSRF protection added on mutating API endpoints (`X-CSRF-Token` header,
    validated server-side) — not covered by the shared conventions, added because a
    same-origin SPA calling a JSON API is a different exposure than the old
    server-rendered form posts it replaces.
  - OpenAPI spec is hand-authored rather than framework-generated, since Flask has
    no built-in equivalent to FastAPI/ASP.NET Core's automatic spec generation.
